...
- **Drive:** add a divergence-free current $J$ to $\mathcal L_\Pi$. This leaves the stationary
  ...
  lifetime.)
- **Memory:** add a restoration map $R$ re-injecting escaped mass back into $\mathcal V$ (reset into
  the basin) → raises $\tau$ without bound as reset rate grows.
...
That **$P=\mathbb E[\tau_\partial]$** — the escape time from the viable set — is, I now think, the
elegant core, and it is *better* than the occupancy version because it gives Drive its due: a
...
occupancy→lifetime correction is foundational and touches canon v9** — I have deposited the gate
