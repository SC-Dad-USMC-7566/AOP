...
> **VERSION 3 (21 Jul 2026), after two Aster red-team passes — verdicts accepted.**
> *First pass* (RED on closure, YELLOW on direction): fixed a **stop-ship bug** (the script optimized `Σ Φ + λ·Σrot` but *reported* only the `Σ Φ` term, so Panel B plotted a step under a "not a jump" title); relabelled the discrete path **hard Viterbi regularization, not soft/annealed**; disclosed that the score is **not grid-invariant**; corrected the transition to **degenerate 1|5 singleton cuts**; denied the spectral operator surrogate status (graph crossing `b=1` ≠ MIP relabel `b≈0.42`); **withdrew the "solved in three fields" headline** (labelled synthesis); lowered grades B and C to FRONTIER.
> *Second pass* (YELLOW direction confirmed; v3 deposit found not-yet-internally-clean — every point reproduced and accepted): the `λ=0` path diagnostics are **non-identifiable under ties** (platform-dependent — `18/9` here, `12/6` on Aster's interpreter) and are now marked so, with the identifiable `λ→0⁺` min-rotation limit (rot 2, 1 event) reported instead; removed the surviving "spectral surrogate / convex relaxation" heading and "annealed-soft-partition reading" label; **Panel B retitled "continuous in λ"** with an explicit note that developmental-time continuity is *not* tested; propagated the excess-entropy correction back into §1; split the merged `a`/`λ` grading row; corrected "six" → **seven tied minimizers** at the exact crossing `b*=0.4206007`; and **added `make_figure()` so the script reproduces the deposited figure** (`--figure`).
> **§4/§9a/§13a stay FRONTIER; canon was never edited.** The Ptaszyński–Esposito line-check was rated GREEN.

- **Author lane:** Claude Science (builder). This is a **proposal** for Prime to verify and Ben to decide — not canon.
- **Date:** 21 July 2026 (v3)
...
2. **The effects-based dead end.** The natural alternative — score the window by a temporal/predictive-information quantity — inherits the §5 difficulty: the excess entropy E = I(past; future) has its standard **stationary invariant / asymptotic form only for a stationary process** [Crutchfield & Feldman 2003], and a developmental window has no stationary "future." Past–future mutual information at a *specified* cut remains definable off-stationarity, but the usual cut-invariant/asymptotic object does not, and the value becomes cut-dependent — enough to make the effects-based route ill-posed as a single window observable. (See §5 for the precise scoping.)
...
where `rot` is the partition-change cost (Hamming distance mod complement) and `λ` weights temporal coherence. This is a chain-structured discrete optimization solved exactly by dynamic programming (Viterbi) — a **hard change-point / partition-path regularization**. It is *not* an annealed or soft-partition construction; the annealing/IB references (§2.3) motivate a possible *future* soft version but do not derive this hard switching-cost functional, which is the builder's own translation (the closer methodological template is the temporal-community formulation, §2.4 / §8).
...
### 3.2 Independent spectral diagnostic (inflated supra-Laplacian analogy — not a surrogate or relaxation)
...
All three checks use deterministic direct computation (matrix inversion/determinants, exhaustive partition enumeration, dynamic programming, eigendecomposition — no statistical estimation) on the canon's own Phase-D two-module Gaussian (Σ = (I + gL)⁻¹, two 3-node modules, inter-module weight `b(t)` ramped through the transition). Numbers below are reproduced by `phaseE_movingMIP_v3_20260721.py`; the figure `phaseE_movingMIP_fig_v3_20260721.png` is regenerated from the same computation via that script's `--figure` flag.

**(A) The obstacle, made concrete [SETTLED — direct computation].** On a window with `b(t)` ramping `0 → 1.4`, the per-slice Φ_MIP argmin is the module cut `{0,1,2}|{3,4,5}` up to the exact crossing `b* = 0.4206007`, then collapses to **degenerate 1|5 singleton cuts** beyond it (at `b*` itself the minimizer set is the module cut *plus* six singleton cuts — seven tied minimizers). A window over `b ∈ [0,1]` straddles the relabeling. This confirms the canon's obstacle is real and locates it — and already exposes that the unnormalized MIP's small-side vulnerability, not a competition between two nontrivial organizations, is what drives the transition in this benchmark.
...
|---:|---:|---:|---:|---:|
| 0.00 | 0.1655 | 0.1655 | *non-identifiable* | *non-identifiable* |
| 0.05 | 0.1702 | 0.1655 | 2 | 1 |
| 0.10 | 0.1750 | 0.1655 | 2 | 1 |
| 0.20 | 0.1845 | 0.1655 | 2 | 1 |
| 0.50 | 0.2069 | 0.2069 | 0 | 0 |

- **At `λ = 0` the path diagnostics are non-identifiable** (Aster 2nd-pass 2.1). With the penalty off, each slice is minimized independently, and the degenerate singleton MIPs across the transition make `rot`/`change-events` an arbitrary tie-breaking artifact of enumeration order — **platform-dependent** (this interpreter prints `18/9`; Aster's printed `12/6`; neither is a property of the objective). Only `J/T` and the deficit part are meaningful at `λ = 0`. The **identifiable** near-zero limit is `λ → 0⁺` with a minimum-rotation tie-break: **rot cost 2, one change event** — the single bounded relabeling.
...
- **The transition is degenerate, not competitive:** module cut `{0,1,2}` → a **1|5 singleton** cut, and at the exact crossing `b* = 0.4206007` the minimizer set is the **module cut plus six 1|5 singleton cuts (seven tied minimizers)** — the unnormalized-MIP small-side vulnerability — *not* a transition between two nontrivially competing organizations. The example does not yet demonstrate tracking between competing partitions.
- The "one relabeling at intermediate `λ`" behavior is a property of *this benchmark and parameter range*, not a general guarantee of the objective.
...
**(C) Spectral diagnostic — not a derived surrogate [FRONTIER].** The deposited supra-Laplacian's leading eigenvalue rises **monotonically** with `a` toward the `a → ∞` time-averaged reference — an *analogous* strong-coupling limit demonstrated on the operator's own terms, not FK's Theorem 7 reproduced (their theorem is about a different, Laplace–Beltrami operator; no transfer argument is claimed). The sweep also stops well short of convergence (`λ₂ = 1.03` at `a = 10` vs the `4.80` limit), so it shows monotone movement, not numerical convergence. Its leading **spatial** eigenmode's L²-mass-per-slice concentrates early and decays. **But this is not validated correspondence to Φ_MIP:** for this exact graph the coupling-Laplacian eigenvalues are analytic (module-difference mode `6b`, within-module modes `3+3b`), so the **graph-spectral crossing is at `b = 1`, not at the Gaussian-MIP relabel near `b ≈ 0.43`**. The mass decay reflects localization where the instantaneous spectral cost is lowest, not a moving-MIP transition. The readout also depends on hand-set `a = 0.8`, a classifier threshold, a lifetime threshold, and the window endpoints, with no robustness sweep deposited.

![Moving-MIP: obstacle, objective vs mis-plot, spectral non-correspondence]({{artifact:art_b62905ec-49b3-44de-b3d1-7630a3085bf2}})

*Figure. (a) The obstacle: the per-slice MIP argmin leaves the module cut at the exact crossing b\* = 0.4206007 — where the module cut and six 1|5 singleton cuts are all tied (seven minimizers). (b) The regularization-value function J/T (solid) is continuous and monotone **in the hyperparameter λ**; the deficit-only component (dashed) — the score reported in an earlier version — is a step, which is why the earlier panel appeared to jump. **This is continuity in λ only; continuity through developmental time / a sliding window through the relabeling is not tested here.** (c) The spectral diagnostic: the leading spatial-mode mass decays, but the analytic graph-spectral crossing (b = 1) does not coincide with the MIP relabel (b ≈ 0.42), so the panel does not validate correspondence between the two constructions.*
...
| The discrete `λ → 0 / λ → ∞` endpoints recover per-slice ↔ frozen scoring | **SETTLED** | Direct property of the builder's finite switching-cost objective (not borrowed from any theorem) |
| The supra-Laplacian's `a → ∞` limit approaches time-averaged (frozen) coupling | **SYNTHESIS (analogous)** | Demonstrated on the deposited operator's own terms; *analogous* to FK Theorem 7 but not that theorem — no transfer argument, and the numerical sweep (`a ≤ 10`) shows monotone movement, not convergence |
...
3. **Degenerate, not competing.** The demonstrated transition is module cut → 1|5 singletons (unnormalized-MIP small-side vulnerability); at the exact crossing b\* = 0.4206007 the tied set is the module cut plus six singletons — **seven minimizers**. A convincing benchmark needs a model with **two genuinely competing non-trivial organizations** (e.g. a 3|3 module cut that yields to a *different* balanced 3|3 cut, or a normalized MIP that resists singleton collapse), and it must expose all tied optimal paths rather than letting enumeration order hide the degeneracy.
...
**Closer discrete precedent to fold in (Aster):** the temporal-community-detection literature treats "snapshot quality + temporal smoothness" as exactly this combinatorial partition-path problem with convex relaxations — e.g. Chen, Kawadia & Urgaonkar, arXiv:1303.7226. *(This reference is Aster's suggestion and has **not** yet been verified against the primary source by the builder — it must be read before it anchors §3.1 in a revision.)* It is a nearer neighbor than the annealing references.

---

## 9. Files

- `AOP_MovingMIP_Build_proposal_v3_20260721.md` — this proposal.
- `phaseE_movingMIP_v3_20260721.py` — runnable, self-contained; reproduces every §4 number by deterministic direct computation (NumPy only) and prints the frontier residue. Run with `--figure` (requires matplotlib) to regenerate `phaseE_movingMIP_fig_v3_20260721.png` from the same computation.
- `phaseE_movingMIP_fig_v3_20260721.png` — the three-panel figure, generated by the script's `make_figure()`.
