...
WHAT THIS SCRIPT DOES (three self-contained checks by deterministic direct
computation - matrix inversion/determinants, exhaustive partition enumeration,
dynamic programming, eigendecomposition; no statistical estimation):
...
literatures; this script is a LABELLED SYNTHESIS of those strategies):
  ...
  * Closer discrete precedent (suggested by Aster review; NOT independently
    verified by the builder - unlike the DOI-resolved entries above): the
    temporal-community literature (snapshot quality + temporal smoothness as a
    combinatorial partition problem with convex relaxations), e.g. Chen, Kawadia &
    Urgaonkar (arXiv:1303.7226). To be checked against the primary source before
    it anchors any revision.
...
def check_B():
    ...
    *_, D = moving_mip(bs, 0.0)
    ...
    for lam in (0.0, 0.05, 0.1, 0.2, 0.5, 1.0):
        ...
        if lam == 0.0:
            # At lambda=0 the penalty is off, so per-slice ties (six degenerate
            # 1|5 singletons across the transition) make rot/events an ARBITRARY
            # tie-breaking artifact of enumeration order - NON-IDENTIFIABLE and
            # platform-dependent (18/9 here, 12/6 on another interpreter). Only
            # the objective and its deficit part are meaningful. [Aster 2nd pass.]
            print(f"     {lam:5.2f}   {obj:12.4f}   {defc:10.4f}    (rot/events non-identifiable under ties)")
        else:
            print(f"     {lam:5.2f}   {obj:12.4f}   {defc:10.4f}  {rotc:7d}   {nch:8d}")
    # the identifiable near-zero-penalty path (lambda->0+ = minimum-rotation
    # tie-break) is the meaningful limit: it takes the single bounded relabeling.
    _, _, _, r0, n0, _ = moving_mip(bs, 1e-9)
    print(f"     lambda->0+ (minimum-rotation tie-break): rot-cost {r0}, change-events {n0}"
          f"  (the identifiable limit)")
    ...
    prev = None
    print("     path (lambda=0.1) - the transition is module-cut -> a SINGLETON 1|5")
    print("     cut. At the exact crossing b*=0.4206007 the minimizer set is the")
    print("     module cut PLUS six 1|5 singleton cuts = SEVEN tied minimizers")
    print("     (unnormalized-MIP small-side vulnerability; the reported path is one")
    print("     arbitrary tie-break):")
...
def check_C():
    ...
    # ANALOGOUS strong-coupling limit (NOT FK's theorem reproduced): FK Thm 7 is
    # about their inflated Laplace-Beltrami operator; here we show the DEPOSITED
    # supra-Laplacian's own leading eigenvalue moves MONOTONICALLY toward the
    # a->inf time-averaged value as a grows. The sweep (a<=10) illustrates
    # monotone movement, NOT numerical convergence - it stops far below the limit
    # (1.03 vs 4.80). No transfer argument to FK is claimed. [Aster 2nd pass 2.6.]
    print("     supra-Laplacian strong-coupling limit (own-terms, analogous to FK Thm 7):")
    ...
    print(f"       a->inf reference (time-averaged L) lambda_2={ref:.4f}"
          f"   (monotone movement, NOT converged at a=10)")
...
# ----------------------------------------------------------------------------
def make_figure(path="phaseE_movingMIP_fig_v3_20260721.png"):
    """Reproduce the deposited three-panel figure from the same computation the
    checks above use. Requires matplotlib (the numerical checks do not). Panels:
    (a) obstacle, (b) objective J/T vs the deficit-only mis-plot, (c) spectral
    diagnostic with the analytic crossing (b=1) marked against the MIP relabel."""
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    foc, alt, grey = "#1f4e79", "#c0561f", "#6b6b6b"

    # (a) running MIP vs module cut over the ramp
    bb = np.linspace(0, 1.4, 60)
    mod_k = [k for k, P in enumerate(PARTS)
             if P in (frozenset({0, 1, 2}), frozenset({3, 4, 5}))][0]
    mip_glob, mip_mod = [], []
    for b in bb:
        S = Sigma(b); defs = [deficit(S, P) for P in PARTS]
        mip_glob.append(min(defs)); mip_mod.append(defs[mod_k])
    mip_glob, mip_mod = np.array(mip_glob), np.array(mip_mod)
    bstar = 0.4206  # exact crossing (module cut == singleton), seven tied minimizers

    # (b) true objective J/T (continuous) vs deficit-only component (steps)
    bs = np.linspace(0.0, 1.0, 21)
    lams = np.linspace(0, 1.0, 101)
    Jobj = np.array([moving_mip(bs, l)[1] for l in lams])
    Jdef = np.array([moving_mip(bs, l)[2] for l in lams])
    perslice, frozen = 0.1655, 0.2069

    # (c) inflated supra-Laplacian leading spatial-mode mass + analytic branches
    bs2 = np.linspace(0.2, 1.4, 31); a = 0.8
    S = supra_L(bs2, a); w, V = np.linalg.eigh(S); Tn = len(bs2)
    def svar(k):
        F = V[:, k].reshape(Tn, N); return F.mean(1).var() / (F.var() + 1e-12)
    kspat = next(k for k in range(1, 4 * N) if svar(k) < 0.3)
    mass = np.linalg.norm(V[:, kspat].reshape(Tn, N), axis=1); mass /= mass.max()

    fig, axs = plt.subplots(1, 3, figsize=(11.5, 3.6))
    ax = axs[0]
    ax.plot(bb, mip_mod, color=alt, lw=2, label="module cut {0,1,2}|{3,4,5}")
    ax.plot(bb, mip_glob, color=foc, lw=2, label="running MIP (per-slice argmin)")
    ax.axvline(bstar, color=grey, ls="--", lw=1)
    ax.text(bstar + 0.02, 0.02, f"MIP leaves module cut\n$b^*\\approx{bstar:.2f}$\n"
            "(→ module cut + 6 singletons\n= 7 tied minimizers)",
            color=grey, fontsize=5.5, va="bottom")
    ax.set_xlabel("inter-module coupling  $b(t)$")
    ax.set_ylabel("integration deficit  $\\Phi(P)$")
    ax.set_title("(a) Obstacle: per-slice MIP argmin\nrelabels inside the window", fontsize=8)
    ax.legend(frameon=False, fontsize=6, loc="upper left"); ax.margins(0.04)

    ax = axs[1]
    ax.plot(lams, Jobj, color=foc, lw=2.2, label="optimized objective $J_\\lambda/T$")
    ax.plot(lams, Jdef, color=alt, lw=1.6, ls="--",
            label="deficit component only\n(earlier mis-reported score)")
    ax.axhline(perslice, color=grey, ls=":", lw=1)
    ax.axhline(frozen, color=grey, ls=":", lw=1)
    ax.text(0.02, perslice + 0.0012, "per-slice optimum", color=grey, fontsize=5.5, va="bottom")
    ax.text(0.98, frozen - 0.0012, "best frozen partition", color=grey, fontsize=5.5,
            va="top", ha="right")
    ax.set_xlabel("regularization weight  $\\lambda$"); ax.set_ylabel("window score")
    ax.set_title("(b) Value function is continuous IN $\\lambda$\n"
                 "(not tested through developmental time)", fontsize=8)
    ax.legend(frameon=False, fontsize=5.5, loc="center right"); ax.margins(0.04)

    ax = axs[2]
    ax.fill_between(bs2, 0, mass, color=foc, alpha=0.22)
    ax.plot(bs2, mass, color=foc, lw=2, label="leading spatial-mode mass")
    ax.axvline(bstar, color=alt, ls="--", lw=1)
    ax.text(bstar - 0.03, 0.55, "MIP relabel\n$b\\approx0.42$", color=alt, fontsize=5.5, ha="right")
    ax.axvline(1.0, color=grey, ls="-.", lw=1)
    ax.text(1.02, 0.55, "graph-spectral\ncrossing $b=1$", color=grey, fontsize=5.5, ha="left")
    ax.set_xlabel("inter-module coupling  $b(t)$"); ax.set_ylabel("spatial-mode mass (norm.)")
    ax.set_title("(c) Spectral diagnostic — crossing ($b{=}1$)\n"
                 "does NOT coincide with MIP relabel", fontsize=8)
    ax.legend(frameon=False, fontsize=5.5, loc="upper right"); ax.set_ylim(0, 1.05); ax.margins(0.04)
...
if __name__ == "__main__":
    ...
    print("  3. At the exact crossing b*=0.4206007 the minimizer set is the module")
    print("     cut PLUS six 1|5 singletons = SEVEN tied minimizers (unnormalized-MIP")
    print("     small-side vulnerability), not two nontrivially competing organiza-")
    print("     tions. Needs a symmetry-broken / two-real-competing-partition")
    print("     benchmark, and to expose all tied paths (Aster 2nd pass, Priority-2")
    print("     item 10; cf. 4.2 'treat degeneracy as data' - track the argmin set).")
