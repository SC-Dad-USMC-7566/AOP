...
## Change run — 15 July 2026 (v1.16, collective-individuality fold: §9a)

Folded one canon movement; its entry is Reconciled as R10 (v1.15→v1.16). The designation
(AOP_Canon_v1_0.md) now names **v1.16** as canonical and holds the Drive master (aop_canon_v1_16.md).
This run adds a single new section, §9a, and one status-table row; no reference was added or removed
and no claim was retracted.

### R10 · Canon movement: v1.15 → v1.16 — reconciled 15 July 2026
- **What.** New section **§9a, "Individuality at the collective scale: the two-level reading,"** placed
  between §9 (present-tense viability) and §10. It carries: (1) [SYNTHESIS] the split of a part's
  "self-interest" on the §9 scope wall into *self-maintenance* (present-tense viability read at the part
  scale — inside AOP) and *private reproductive stake* (differential continuation across generations —
  above the wall), independent because they live on two levels; durable collectives keep the first and
  surrender the second; cancer is the part re-asserting the second. Offered as the cleaner statement of
  why the §9 wall sits where it does, not a new dimension. (2) [SYNTHESIS] the synchronic reading — a
  Φ_MIP > 0 collective (§4) is one higher persister with its own four-axis profile — connected to the
  part's viability. (3) [SETTLED, cited context] the across-generation transition mechanism
  (single-cell bottleneck, policing, life-cycle fitness-decoupling, major-transitions staircase),
  recorded as above-wall external biology that motivates the scope wall and flagged as a Ladder
  propagation-bus item, explicitly not a persistence claim. (4) [FRONTIER] the collective
  living-threshold — does a collective hold a decoupled model of the *collective's* own viability
  (§11a at collective scale)? — bottlenecked on the nested-level, non-stationary Φ_MIP extension §4
  scopes out; inherits the F2 status of the §9 higher-individual route. A §12 status-table row records
  the split grade. Masthead → v1.16 (15 July 2026) + closing changelog paragraph.
- **Why.** The user asked that the *science* discussed in a pedagogical companion ("Me and We") — the
  major-transitions / we→me material — be folded into the canon, on the confirmed scope partition: new
  synthesis + frontier admitted as canon; the transitions biology cited as above-wall context, not
  asserted as AOP claims. This preserves the §9 scope wall the rest of the canon depends on.
- **Grade.** No grade promotions. Two synthesis moves (the two-dial split; synchronic individuality),
  one settled-external-biology citation block, one frontier claim (collective living-threshold, F2).
  The frontier claim inherits, and does not weaken, the v1.15 F2 downgrade.
- **Verification.** Aktipis et al. 2015 verified in full-text body ("a breakdown of the central
  features of cooperation … five foundations of multicellularity"). Michod 2007, Queller & Strassmann
  2009, Grosberg & Strathmann 2007 verified at the abstract level (cited results are abstract-level
  claims; full bodies not retrieved — PMC bronze / closed). Frank 1995 and Hammerschmidt et al. 2014
  closed-access, not re-verified this session, carry the standard-reference marker in-text. No reference
  entries added or removed — all six sources were already in the list from the v1.13/v1.14 life-
  threshold fold ("cite, don't create").
- **Downstream.** The §9a transition-mechanism material and the collective living-threshold both point
  *up* to the Ladder and are flagged as propagation-bus items; the standing v1.12/v1.14/v1.15
  hub-classification propagation note (still unposted) now additionally covers §9a. The F2 seam is
  reaffirmed as the shared open problem of the life/individuality material and the Ladder rebuild.
- **Status.** Reconciled 15 July 2026. Canonical master to be held on Drive (aop_canon_v1_16.md).
