...
**T3 — AOP beats the single-axis rival (margin 0.10, both sub-criteria met).** On the essentiality-AUROC criterion the frozen scorer records AOP AUROC = 0.8476 and rival AUROC = 0.7476 (`external_benchmark_results.json`), a margin of 0.1000 to four decimals, meeting the preregistered ≥0.10 bar. (The "0.0999…" an earlier draft reported was IEEE-754 representation noise from subtracting these two decimals in float64, not a sub-threshold empirical result. Note also that FBA admits alternate optimal flux distributions, so the exact AUROCs — particularly the rival's, which depends on the specific WT flux vector — are solver-path dependent at the third decimal; the frozen script fixes the solver so prime's re-run reproduces the recorded values, and the ≥0.10 margin is robust to this at reporting precision.) The coalition sub-criterion also holds cleanly: the rival (flux strength) scatters the synthetic-lethal members across its strength ranks 11–117 of 136, so it has no way to flag a pair as jointly essential. The single-axis strength rival is nonetheless a **respectable** essentiality classifier on real metabolism (AUROC 0.75) — much stronger than it was on the toy model, where it inverted the ranking. AOP wins on both sub-criteria, but the rival's real-metabolism competence is itself part of the honest picture.
