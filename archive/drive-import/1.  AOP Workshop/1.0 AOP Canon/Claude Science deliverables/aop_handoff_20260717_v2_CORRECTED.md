...
- **Submission package (current, 15 Jul):** a full derived Royal Society *Interface Focus* Perspective, in Drive at
  ...
  - `figures/` — 12 PNGs at 300 dpi: **Fig 1–6 main + Fig S1–S6 SI** (per my own Drive crawl of the `figures/` subfolder). Note: three further SI figures — `figS7_gate_stakes.png`, `figS8_substitutability.png`, `figS9_realsystem.png` — exist as **loose files in the Submission Package root, not inside `figures/`**. So the total SI figure count is 9 if those are counted; the manuscript's own figure manifest should be the authority on final numbering. (The pre-existing 16-Jul handoff manifest's "Fig S1–S7 inside figures/" phrasing is inconsistent with the actual folder contents.)
...
## 3. The OAI corrections to adjudicate

For each, the new tab should decide **defect vs. disagreement** against the actual v1.16 text (not from memory).

### The six P0 "stop-ship" items (exactly as named in the master plan's Priority-order section)

The master plan's own Priority-order paragraph tags **only these six** as P0:
...
2. **Proxy-label mismatches.** OAI: stop equating raw MI = boundary strength; entropy production = free-energy throughput; excess entropy = stored memory; total correlation = irreducible integration. Label each as the proxy it is (dependence / dissipation / predictive-dependence / multivariate interdependence).
3. **"Choice-free" language ("Drive and Memory are choice-free").** OAI: restate as "require no *additional* spatial/component cut once a process representation, time grain, and reversal convention are fixed."
...
### Also required, but as Phase-1 "immediate corrections" — NOT tagged P0 by the source

These appear in the master plan's Phase 1 (§ "Required corrections") but the Priority-order section does **not** list them as stop-ship. Treat as important but lower-urgency than the six above:

- **"Energy is the hub."** OAI: reframe as an organizing hypothesis unless the graph among the four targets actually supports hub status; reliability is an external consequence, not automatically a fifth axis.
- **Gravity / screenability / light-cone.** OAI: quarantine from the scientific core until reviewed as a separate boundary-taxonomy.
- **Worked cases (crystal/flame/spore/atom/star).** OAI: relabel as **diagnostic archetypes** unless all declarations and quantities are actually computed.

*(Phase 1 has ten required corrections in total; the remainder overlap the six P0 items above. Read the master plan's Phase 1 section directly for the full list.)*
...
**Reference-verification debt** — ⚠️ **the two in-window source docs disagree materially; the new tab MUST reconcile against `aop_reference_punchlist.md`, which is authoritative.** The counts below are quoted from both so the discrepancy is visible:
- **Total refs:** README says 52; the punch-list is the authority — re-count it.
- **verified-in-body:** README says 25; the 16-Jul handoff manifest says 31.
- **abstract-verified:** both say ~14.
- **record-only:** both say **5**, **flagged NOT acceptable for submission** under the charter standard: Watanabe 1960, Marquardt 1970, Conant & Ashby 1970, Nicholson & Dupré 2018, Muller 1964. Each is non-load-bearing background or has a softening path in the punch-list.
- **needs-user-PDF:** ⚠️ **the sources differ by ~4×** — the README says **8**, the handoff manifest says **2** (naming only Ashby 1960 and Parfit 1984, both print-only and non-load-bearing). This is the single largest source disagreement and it changes the apparent severity of the reference debt: if the true figure is 8, six additional print-only sources are outstanding and may not all be non-load-bearing. **Resolve this against the punch-list before assuming the debt is small.**
